...
abstract: |
  ...
  This tool will allow a reverse engineer to break their work into smaller pieces. 
...
This work is an extension of work done in a previous class [@fordon2016]

I want to do something very similar to in-memory fuzzing.
I want to be able to describe the expected behavior of the system as well as the expected process state.
...
By properly describing the pre-conditions and post-conditions for a given portion of the code, we can create all valid process states.
We then instrument a binary to enable the testing of these process states and the expected behavior as described by post-conditions.
...
By using stdin to set process-state, we can leverage existing fuzzing tools.
The new instrumented binary will behave like a normal program that reads from stdin.
The process is expected to terminate quickly without error.

Using this technique, we can isolate the relevant section of a program and specific, security-related, conditions.
This approach has several advantages:

* It is easier to test difficult to emulate systems since you can avoid difficult sections (e.g. database calls and system I/O).
...
The the user must determine what the possible initial/end state are.
...
This tool is also useful in that it allows a reverse engineer to test their assumptions.
...
However, mutational fuzzers can find bugs that the generational may not [@demott2007]
...
The control flow graph represents all the possible paths of execution for a binary.
...
However, coverage is still useful in discussing the effectiveness of fuzzing.

Another important distinction is if the fuzzer is aware of the programs structure and/or the coverage during the execution.
Depending on the fuzzers awareness it is either: white-box, black-box, or grey-box.
...
This usually allow very fast execution, but does not lead to interesting code paths since it is not aware that paths even exist<!--[@citationNeeded]-->.
...
They could want to know how the program would behave in similar, but slightly different, scenarios.
This is not traditional fuzzing since it does not modify a program's inputs.
It is still interesting to see how the process behavior can change when portions of its memory is modified.
...
Due to this, in-memory fuzzing has not gained wide-spread adoption<!--[@citationNeeded]-->.
...
This was the first published general purpose evolutionary fuzzer that does not require source code.
...
Without prior knowledge of the inputs structure, they could effectively test complex inputs.
This was useful since it allowed better testing of programs were the source code was not available without reverse engineering.

Since EFS, there have been many other mutation based fuzzing tools <!--[@citationNeeded] [ @citationNeeded]-->.
In recent years, American Fuzzy lop has gained particular popularity due to its practical approach and being open source.


#### American Fuzzy Lop ###

American Fuzzy Lop (AFL) is an evolutionary mutational fuzzing tool <!--[@citationNeeded]--> that works with programs that use a single input file.
AFL maintains a set of inputs that it mutates and evaluates.
...
To bootstrap a library, a new program is written that reads and necessary inputs and calls the function under test.
Similar bootstrapping can be performed to target network interfaces of a program.

![Example control flow graph for with an example path and edge counts \label{figCFGPath}](images/afl_edge_counts.png){height=50%}

Figure \ref{figCFGPath} shows a single execution of a program and the branches taken.
AFL would represent this coverage by counting the number of times each edge on the control flow graph was traversed.
The table in the image above also shows the expected edge counts for the shown code path.
...
As the QEMU translates the code blocks to the native system, the branch tracking code is injected.
...
In 2008, Microsoft introduced SAGE [@godefroid2008automated]
...
Constraint solving with Symbolic execution work by representing the process as a formal system.
...
By representing the system in this way, it is easier to solve for system constraints.
...
The this could be solved by symbolically executing and adding constraints and solving them.
...
Then Driller attempts to perform constraint solving to discover new code paths.
...
ELF executables describe how a process should be then loaded into memory.
...
The goal of their this injection is to minimally modify the existing binary while still allowing the executable to work with standard protections like ASLR and NX.
...
A section may not be mapped to a segment, and multiple sections can be mapped to the same segment.
...
But, it cannot call any new functions.
...
But emulation alone does not fundamentally increase the ability to find difficult errors.
...
Avionics systems, like many embedded systems, often cycle through the steps of input, compute, output.
I wanted to be able to test just compute section under emulation.
...
Preferably, this is defined by a formal spec or a contract.
...
If the program breaks this expected behavior is called a bug.
...
The definition of a vulnerability requires an understanding of the users security needs.  
...
The RE determines these through normal reverse engineering.
...
Perhaps the program will operate to spec as long as the inputs are "valid", but the calling code can provide inputs that are not to spec.
...
Without a formal specification the programmer vs caller assumptions are likely to vary andn create space for bugs/vulnerabilities.
...
I think this is the natural way to approach the original problem and has broader implications.
...
Creating an instrumented binary based on a raw binary (as exist in most embedded systems) would have been more effective.
...
In order to do this, I need to supply information to to the instrumentation.
...
With the reverse engineering knowledge I can use my project's tool (fuzzyprobe) to create a new binary.
...
And, this instrumented binary can use stdin to specify portions of the program state.
...
The goal of this paper is to fuzz a set of system states for process.
I do this by defining subsection of the original process and describing valid system states.
This project focused on memory, but other system state could also be exposed.
...
When the instrumented program runs, it will set any fixed values we have specified and then populate the mutable areas with values from stdin.
...
The new\_main function will become main, and the end\_check will be run after the section under test.
...
This main function is responsible for jumping to to the section that is under test.
...
This input file is a seed input that tools like AFL can use to begin fuzzing.
The file is filled with zeros and is exactly as long as the expected input.

The instrumented binary has been modified using the ERESI method describe previously.
New sections were added to the beinging of the executable segment and the end of the data segment.
The PLT was modified to redirect main to injected main.
...
However, the goal of this project is not to compete with these tools, but build a tool that will work with them.
Really this tool is targeted at patching binaries in order to make them 

I did not find a similar tool in my search.
...
Manticore perform constraint solving by defining hooks to entry/exit points and defining system constraints.
...
However, Manticore is solving a different problem; They are finding a specific set of inputs that solve a given set of constraints.
...
In memory fuzzing also requires understanding the underlying code through reverse engineeering.
Corelan produced a set of tools "Tracer.py" and "InMemoryFuzzer.py" to ease the amount of reverse engineering required.
The tool is very usable, but uses its own mutation engine and did not seem like it would be easily extended for emulation.
...
And Acri  presented a tool that connects to the GNU debugger gdb and uses this to attach to invoke a process.
This approach seemed useful, in that gdb could be invoked with QEMU, but the tool would have to be extended.
...
This tool resemble an in-memory fuzzer in that it allows the user to fuzz the process state and not just the inputs.
...
The current state is a good proof of concept, but it has a way to go before it is a useful tool.
I am going to be developing this tool in order to perform fuzzing on an embedded system.
I will be adding new features to the tool on an as needed basis, but anticipate many needed changes

## Fixing symbol loading issues ##

The injection method we use will not work if the original binary does not use "read" and "malloc" from glibc.
There are several ways to deal with this.
I could hijack an existing, unused function/library, or look into incorporating a relocation engine into libelsh.
...
I think being able to populate RPC style objects would be helpful for non-trivial binaries.
...
The long jump instruction was so long it extended into another piece of code I would like to test.
...
The actual section under test could be modified before each instruction call to orchastrate this.
The major drawback of this approach is that it would execute much slower. 
...
<i>Thank you Dr. Mateti for your thoughtfull advice and encouragement.</i>
